...
class MaturedDecision:
    ...
    def prediction_residual(self)->float:
        """Observed return minus the prediction frozen at decision time."""
        return float(self.realized_return-self.predicted_return)
...
class ValuationLedger:
    ...
    def record_decision(self,decision_id,step,phi,*,chosen_by,candidate_id,predicted_return:float=0.0):
        self._pending.append(PendingDecision(
            str(decision_id),int(step),str(candidate_id),
            phi.detach().cpu().to(torch.float64).clone(),str(chosen_by),
            float(predicted_return),0.0,
        ))
    def observe_cost_step(self,step,*,task,probe,edit,maintenance,reopen):
        ...
        for row in self._pending:
            if step>=row.decision_step+self.horizon:
                self._matured.append(MaturedDecision(
                    row.decision_id,row.decision_step,row.candidate_id,row.phi.clone(),
                    row.chosen_by,row.predicted_return,row.accumulated_return,
                ))
            else:
                still.append(row)
        self._pending=still
        ...
        for row in self._pending:
            if row.decision_step<=step<=row.decision_step+self.horizon-1:
                updated.append(PendingDecision(
                    row.decision_id,row.decision_step,row.candidate_id,row.phi,row.chosen_by,
                    row.predicted_return,row.accumulated_return-cost,
                ))
            else:
                updated.append(row)
