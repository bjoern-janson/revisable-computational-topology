...
from .lifetime import BobLifetime,ScientificExecutionNotAuthorized
from .types import ExecutionAuthorization

def _parser():
    p=argparse.ArgumentParser(prog='rct.bob'); sub=p.add_subparsers(dest='cmd')
    sub.add_parser('status')
    a=sub.add_parser('audit-world')
    v=sub.add_parser('validate'); v.add_argument('--steps',type=int,default=8)
    r=sub.add_parser('run'); r.add_argument('--execute-lifetime',action='store_true'); r.add_argument('--design-commit'); r.add_argument('--implementation-commit'); r.add_argument('--authorization-token'); r.add_argument('--output-dir')
    return p
...
def main(argv=None):
    ...
    if args.cmd=='status':
        ...
        print('Implementation         IMPLEMENTED_NOT_EXECUTED')
        print('Scientific execution  AUTHORIZED / UNEXECUTED')
    ...
    if args.cmd=='run':
        if not args.execute_lifetime or not args.design_commit or not args.implementation_commit or not args.authorization_token or not args.output_dir:
            print('scientific run requires explicit --execute-lifetime, full authorization identity, and --output-dir',file=sys.stderr); return 2
        auth=ExecutionAuthorization(args.design_commit,args.implementation_commit,args.authorization_token)
        try:
            b=BobLifetime(conformance_only=False,output_dir=args.output_dir); b.run_lifetime(auth,repo_root='.')
